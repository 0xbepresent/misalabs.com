...
class Router(object):
    ...
    def add(self, rule, method, target, name=None):
        ...
        try:
            ...
            self.dynamic.append(
                (re.compile("(^%s$)" % flat_pattern), [(match, target)])
            )
        return match
...
class Bottle(object):
    ...
    def _cast(self, out, peek=None):
        ...
        if isinstance(first, unicode):
            return imap(
                lambda x: x.encode(response.charset), itertools.chain([first], out)
            )
...
class BaseRequest(DictMixin):
    ...
    def json(self):
        ...
        exhaustion."""
        if (
            "application/json" in self.environ.get("CONTENT_TYPE", "")
...
class HeaderProperty(object):
    def __init__(self, name, reader=None, writer=str, default=""):
        self.name, self.reader, self.writer, self.default = (
            name,
            reader,
            writer,
            default,
    ...
    def __get__(self, obj, cls):
        ...
        return (
            self.reader(value) if (value and self.reader) else (value or self.default)
        )
...
class SimpleTALTemplate(BaseTemplate):
    ...
    def prepare(self, **options):
        depr(
            "The SimpleTAL template handler is deprecated"
